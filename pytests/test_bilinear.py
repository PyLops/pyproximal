...
par2 = {"n": 21, "m": 11, "k": 5, "imag": 1j, "dtype": "complex64"}  # complex
...
def test_lrfactorized(par):
    ...
    U = (
        np.random.normal(0.0, 1.0, (par["n"], par["k"]))
        + par["imag"] * np.random.normal(0.0, 1.0, (par["n"], par["k"]))
    ).astype(par["dtype"])
    V = (
        np.random.normal(0.0, 1.0, (par["m"], par["k"]))
        + par["imag"] * np.random.normal(0.0, 1.0, (par["m"], par["k"]))
    ).astype(par["dtype"])
...
def test_lrfactorizedoperator(par):
    ...
    Op = Diagonal(np.arange(par["n"] * par["m"]) + 1.0)
