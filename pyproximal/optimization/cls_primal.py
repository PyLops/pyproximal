...
class ProximalGradient(Solver):
    ...
    def _print_setup(self, epsg_print: str, xcomplex: bool = False) -> None:
        ...
        tau_str = (
            f"{self.tau[0]:4.2e}"
            if self.tau.size == 1
            else ", ".join(f"{tau:4.2e}" for tau in self.tau)
        )
    ...
    def _print_step(
        ...
        msg = (
            f"{self.iiter:6g}        "
            + strx
            + f"{pf:10.3e}  {pg:10.3e}  {self.pfg:10.3e}  {tau_str}"
        )
        print(msg)
...
class AndersonProximalGradient(Solver):
    ...
    def _print_setup(self, epsg_print: str, xcomplex: bool = False) -> None:
        ...
        strpar1 = f"tau = {tau_str}\t\tepsg = {epsg_print}\t\tniter = {self.niter}"
    ...
    def _print_step(self, x: NDArray, pg: float | None, epsg_prev: int) -> None:
        ...
        msg = (
            f"{self.iiter:6g}        "
            + strx
            + f"{self.pf:10.3e}  {pg:10.3e}  {self.pfg:10.3e}  {tau_str}"
        )
        print(msg)
...
class GeneralizedProximalGradient(Solver):
    ...
    def _print_setup(self, epsg_print: str, xcomplex: bool = False) -> None:
        ...
        strpar1 = (
            f"tau = {float(self.tau):4.2e}\tepsg = {epsg_print}\tniter = {self.niter}"
        )
        print(strpar)
        print(strpar1)
        print("-" * 65 + "\n")
...
class ADMM(Solver):
    ...
    def _print_setup(self, xcomplex: bool = False) -> None:
        ...
        strpar1 = f"tau = {float(self.tau):6e}\tniter = {self.niter}"
        strpar2 = f"gfirst = {self.gfirst}\t\ttol = {self.tol}"
        print(strpar)
        print(strpar1)
        print(strpar2)
...
class PPXA(Solver):
    ...
    * for :math:`k = 1, \ldots`
    ...
    def step(
        ...
        """
        # store current solution prior to update
        # for tolerance check
        self.xold = x
    ...
    * for :math:`k = 1, \ldots`
    ...
    def setup(  # type: ignore[override]
        ...
        x_bar : :obj:`numpy.ndarray`
            Initial guess for averaged model vector
        y : :obj:`numpy.ndarray`
            Initial guess for additional model vector
    ...
    def step(
        ...
        x : :obj:`numpy.ndarray`
            Current model vector to be updated by a step of the
            ConsensusADMM algorithm
        x_bar : :obj:`numpy.ndarray`
            Current averaged model vector to be updated by a step of the
            ConsensusADMM algorithm
        y : :obj:`numpy.ndarray`
            Additional model vector to be updated by multiple steps of
        ...
        -------
        x : :obj:`numpy.ndarray`
            Updated model vectors
        x_bar : :obj:`numpy.ndarray`
            Updated averaged model vector
        y : :obj:`numpy.ndarray`
            Updated additional model vector
        ...
        # proximal steps
        x = self.ncp.stack(
            [self.proxfs[i].prox(x_bar - y[i], self.tau) for i in range(self.nprox)]
        )
        x_bar = self.ncp.mean(x, axis=0)
        y = y + x - x_bar
    ...
    def run(
        ...
        while self.iiter < niter and not self.tolbreak:
            ...
            x, x_bar, y = self.step(x, x_bar, y, showstep)
            self.callback(x_bar)
            # check if any callback has raised a stop flag
            stop = _callback_stop(self.callbacks)
            if stop:
    ...
    def solve(  # type: ignore[override]
        ...
        x, x_bar, y = self.run(x, x_bar, y, niter, show=show, itershow=itershow)
        self.finalize(65, show)
        return x, x_bar, y, self.iiter, self.cost
