...
n, m, k = 100, 90, 10
X = np.maximum(np.random.normal(0, 1, (n, k)), 0) + 1.0
Y = np.maximum(np.random.normal(0, 1, (k, m)), 0) + 1.0
...
Xin = np.maximum(np.random.normal(0, 1, (n, k)), 0) + 1.0
Yin = np.maximum(np.random.normal(0, 1, (k, m)), 0) + 1.0
Hop = pyproximal.utils.bilinear.LowRankFactorizedMatrix(Xin, Yin, A.ravel())

###############################################################################
# We are now ready to run the PALM algorithm
snr_palm = []
Xpalm, Ypalm = pyproximal.optimization.palm.PALM(
    Hop,
    nn1,
    nn2,
    Xin.ravel(),
    ...
    niter=2000,
    show=True,
    callback=lambda x, y: callback(x, y, n, m, k, A, snr_palm),
)
...
axs[3].axis("tight")
axs[4].imshow(A - Apalm, cmap="gray", vmin=-0.1, vmax=0.1)
...
snr_palmbt = []
Xpalmbt, Ypalmbt = pyproximal.optimization.palm.PALM(
    Hop,
    nn1,
    nn2,
    Xin.ravel(),
    ...
    niter=2000,
    show=True,
    callback=lambda x, y: callback(x, y, n, m, k, A, snr_palmbt),
)
...
axs[3].set_title("Reconstructed")
axs[3].axis("tight")
axs[4].imshow(A - Apalmbt, cmap="gray", vmin=-0.1, vmax=0.1)
axs[4].set_title("Reconstruction error")
axs[4].axis("tight")
fig.tight_layout()
...
snr_ipalm = []
Xipalm, Yipalm = pyproximal.optimization.palm.iPALM(
    Hop,
    nn1,
    nn2,
    Xin.ravel(),
    ...
    niter=2000,
    show=True,
    callback=lambda x, y: callback(x, y, n, m, k, A, snr_ipalm),
)
...
axs[3].set_title("Reconstructed")
axs[3].axis("tight")
axs[4].imshow(A - Aipalm, cmap="gray", vmin=-0.1, vmax=0.1)
axs[4].set_title("Reconstruction error")
axs[4].axis("tight")
fig.tight_layout()
